...
First and foremost, it is necessary to create the EC2 instance that will host our web service,
named `caa3-ami-1` with the tag `aperez-b`.

See @fig-review-create-instance for more details.
...
```bash
sudo yum update -y                             # <1>
sudo yum upgrade -y                            # <1>
sudo hostnamectl set-hostname aperez-b-master  # <2>
sudo yum install httpd -y                      # <3>
sudo systemctl enable --now httpd              # <3>
```
1. Update the system.
2. Set hostname to `aperez-b-master`.
3. Install and set up `httpd`.
...
3. Images from screenshots I took in the previous CAA.

At this point our website is up and running at [http://ec2-3-94-211-15.compute-1.amazonaws.com](http://ec2-3-94-211-15.compute-1.amazonaws.com). We can check that it is working with `JMeter` (see @fig-test-jmeter-ex1).
...
See @fig-test-jmeter-ex2 to verify that the websites are served properly from the `frontend`.
...
For this last part I merely followed the 5-module tutorial from the post [*Deploy WordPress with Amazon RDS*](https://aws.amazon.com/getting-started/hands-on/deploy-wordpress-with-amazon-rds/) carefully to properly install and configure `WordPress`.

See @fig-aws-ex3 to check the settings of EC2 and RDS after following the aforementioned tutorial.

### WordPress installation

See @fig-wordpress-installed-ex3 to see how the WordPress site looked before and after setting up the database and the Wordpress app itself.
...
After configuring the database and the VM, it is time to log in with a new WordPress user. In my case I created a new user `aperez-b` with my `UOC` email address and started adding blog posts to the site. I added a total of six entries, with various types of content and media. Some have lists, images, links, quotes, video, audio, and so on (see @fig-wordpress-post-overview).

::: {.callout-note}
As it is hard to show the WordPress site with just screenshots, I recorded a [short video](./wordpress-sample-ex3.webm) showcasing every post (thus proving that it all works as expected)
:::

### WordPress performance

See @fig-pagespeeds-ex3 to view the result of the test performed by [PageSpeed Insights](https://pagespeed.web.dev/).

::: {.callout-warning}
In @fig-pagespeeds-ex3, the url appears to be different from all other screenshots from this exercise. This is because I had to enable the site to be accessed by anyone (at first I had set it up so that only my IP could access the website), and as a result a new Public IP address was assigned
:::

{{< pagebreak >}}
## Conclusion

In this CAA I have worked with AWS for the first time, and I cannot say it was a piece of cake.
Nevertheless, it was a fun journey.The overall experience of working with it to deploy applications
has been a valuable learning opportunity. By understanding the technologies for the deployment of
cloud computing infrastructures securely and efficiently, I was able to deploy a balanced proxy
using Apache2 with a front-end and two workers, as well as a full-fledged WordPress server. Overall,
the experience has provided me with a solid foundation in cloud computing that I can build upon in
future projects.
...
![Add instance preview](./img/review-create-instance.png){#fig-review-create-instance width=60%}

::: {#fig-website-ex1 layout-ncol=2}

![JMeter plan result](./img/test-jmeter-ex1.png){#fig-test-jmeter-ex1}

![Web server up and running](./img/website-running-ex1.png){#fig-website}

Website running on browser and in JMeter
:::

::: {#fig-lbmethod layout-ncol=2}

![`lbmethod=byrequests` - 50 failed requests](./img/lbmethod-byrequests.png){#fig-lbmethod-byrequests}

![`lbmethod=bytraffic` - 30 failed requests](./img/lbmethod-bytraffic.png){#fig-lbmethod-bytraffic}

Test the command `ab -n 100 -c 10 <FrontendPublicIP>` with two balancing algorithms
:::

![`balancer-manager` working](./img/balancer-manager-ex2.png){#fig-balancer-manager width=70%}

::: {#fig-pages-ex2 layout-ncol=2}

![Page from `worker1` accessed from `frontend`](./img/worker1-browser-ex2.png){#fig-pages-worker1-ex2}

![Page from `worker2` accessed from `frontend`](./img/worker2-browser-ex2.png){#fig-pages-worker2-ex2}

Pages served by each worker balanced from the `frontend`
:::

![JMeter plan result](./img/test-jmeter-ex2.png){#fig-test-jmeter-ex2 width=70%}

::: {#fig-aws-ex3 layout-ncol=2}

![EC2 settings](./img/wordpress-instance-ex3.png){#fig-wordpress-instance-ex3}

![RDS settings](./img/wordpress-db-ex3.png){#fig-wordpress-db-ex3}

EC2 and RDS configuration
:::

::: {#fig-wordpress-installed-ex3 layout-ncol=2}

![pre-setup](./img/wordpress-ex3.png){#fig-wordpress-installed-pre}

![post-setup](./img/wordpress-installed-ex3.png){#fig-wordpress-installed-post}

WordPress pre-setup and post-setup
:::

![[PageSpeed Insights](https://pagespeed.web.dev/) report](./img/pagespeeds-insights-ex3.png){#fig-pagespeeds-ex3 width=70%}

![WordPress post overview](./img/wordpress-post-overview-ex3.png){#fig-wordpress-post-overview width=70%}
